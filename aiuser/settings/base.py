...
class Settings(
    ...
    async def usermlist(self, ctx: commands.Context):
        ...
        embed.description = "\n".join(
            [f"`{model}`" for model in self.avaliable_models])
    ...
    async def user_model(self, ctx: commands.Context, model: str):
        ...
        **参数**
        ...
        if model not in self.avaliable_models and ctx.author.id != self.bot.owner_id:
            return await ctx.send(f":warning: 无效的模型!\n请使用以下模型之一: {', '.join(self.avaliable_models)}", delete_after=30)
    ...
    async def optoutuser(self, ctx: commands.Context, *, member: discord.Member):
        ...
        if member.id in optout:
            return await ctx.send(f"{member.mention} You are already opted out.")
        optin = await self.config.optin()
        if member.id in optin:
            optin.remove(member.id)
            await self.config.optin.set(optin)
    ...
    async def optin_by_default(self, ctx: commands.Context):
        ...
        """
        if len(ctx.guild.members) > 150000:
            ...
            return await ctx.send(
                "You cannot enable this setting for servers with more than 150000 members."
            )
